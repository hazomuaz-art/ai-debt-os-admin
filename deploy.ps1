...
# Tracked paths that still exist on disk. Filtering by Test-Path drops any
# files deleted in the working tree but not yet committed (the local index can
# be stale when a commit failed) — so tar never chokes on missing files, and
# deletions still propagate (absent here + `rm -rf src` on the server).
git ls-files | Where-Object { Test-Path -LiteralPath $_ } | Set-Content -Path $list -Encoding ascii
