    ...
    <div className="flex-1 overflow-y-auto px-8 pb-8 space-y-6 bg-[#f0f4f8] font-sans text-slate-800" dir="rtl">
      ...
      <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 flex items-center justify-between mt-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-[#1e3e50] mb-1">فريق العمل (المحصلون)</h1>
            <p className="text-slate-500 text-sm">متابعة وإدارة أداء محصلي الديون في شركتك.</p>
          </div>
        </div>
      </div>
      ...
      {collectorStats.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {collectorStats.map((c) => {
            ...
            return (
              <div key={c.id} className="bg-white rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow overflow-hidden flex flex-col">
                <div className="p-6 border-b border-slate-50 flex items-start justify-between">
                  <div className="flex items-center gap-4">
                    <div className="w-14 h-14 bg-gradient-to-br from-[#1e3e50] to-slate-600 text-white rounded-2xl flex items-center justify-center text-xl font-bold shadow-sm shrink-0">
                      {c.full_name?.charAt(0) ?? '?'}
                    </div>
                    <div>
                      <h3 className="font-bold text-[#1e3e50] text-lg">{c.full_name ?? 'بدون اسم'}</h3>
                ...
                <div className="p-6 grid grid-cols-3 gap-4 bg-[#fcfdfd] flex-1">
                  <div className="bg-white border border-slate-100 rounded-xl p-3 text-center">
                    ...
                    <p className="text-xl font-bold text-[#1e3e50] font-mono">{c.totalAssigned}</p>
                    <p className="text-[10px] font-bold text-slate-400 mt-1">ملف مسند</p>
                  </div>
                  ...
                  <div className="bg-white border border-slate-100 rounded-xl p-3 text-center">
                    ...
                    <p className="text-[10px] font-bold text-slate-400 mt-1">مُغلق بنجاح</p>
                  </div>

                  <div className="bg-white border border-slate-100 rounded-xl p-3 text-center">
                    <div className="w-6 h-6 bg-slate-50 text-slate-500 rounded-full flex items-center justify-center mx-auto mb-2">
                      <span className="font-bold text-[10px]">SAR</span>
                    </div>
                    <p className="text-sm font-bold text-[#1e3e50] font-mono truncate px-1 mt-1.5">{formatCurrency(c.totalBalance, 'SAR')}</p>
                    <p className="text-[10px] font-bold text-slate-400 mt-1">إجمالي المحفظة</p>
                  </div>
                </div>

                <div className="p-6 border-t border-slate-50 bg-white">
                  <div className="flex justify-between text-xs font-bold mb-2">
                    <span className="text-[#1e3e50]">معدل الإنجاز (التحصيل)</span>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-16 text-center">
          ...
          <div className="font-bold text-xl text-[#1e3e50] mb-2">لا يوجد محصلون في فريقك حالياً</div>
          <p className="text-slate-500 text-sm">قم بدعوة الموظفين للنظام وتعيين دور (محصل) لهم ليظهروا هنا.</p>
        </div>
      )}
    </div>
  )
