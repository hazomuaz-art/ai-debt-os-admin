...
function AddPortfolioModal({ onSaved }: { onSaved: () => void }) {
  ...
  return (
    <div className="fixed inset-0 bg-[#0e7a54]/40 backdrop-blur-sm flex items-center justify-center z-[9999] p-4 animate-in fade-in" >
      <div className="bg-[#151a23] border border-[#222a36] rounded-2xl w-full max-w-lg shadow-2xl animate-in slide-in-from-bottom-4">
        ...
        <div className="flex items-center justify-between p-6 border-b border-[#222a36] bg-[#0d1117] rounded-t-2xl">
          <div className="flex items-center gap-3">
            ...
            <h2 className="font-bold text-white text-lg">إضافة محفظة / مشروع جديد</h2>
          </div>
          <button onClick={() => setOpen(false)} className="text-[#5f6b7e] hover:text-slate-300 hover:bg-[#222a36] p-2 rounded-lg transition-colors">
            <XCircle size={20} />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-bold text-slate-300 mb-2 ps-2">الاسم (عربي) *</label>
              <input required className="w-full bg-[#0b0e14] border-none text-white rounded-xl px-4 py-3 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-[#0e7a54]" 
                value={form.name_ar} onChange={e => setForm(p => ({ ...p, name_ar: e.target.value }))} placeholder="موبايلي"  />
            </div>
            <div>
              <label className="block text-sm font-bold text-slate-300 mb-2 ps-2">الاسم (إنجليزي) *</label>
              <input required className="w-full bg-[#0b0e14] border-none text-white rounded-xl px-4 py-3 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-[#0e7a54] text-end" 
                value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} placeholder="Mobily" dir="ltr" />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-bold text-slate-300 mb-2 ps-2">الرمز القصير *</label>
              <input required className="w-full bg-[#0b0e14] border-none text-white rounded-xl px-4 py-3 text-sm font-mono uppercase focus:outline-none focus:ring-2 focus:ring-[#0e7a54] text-end" 
                value={form.code} onChange={e => setForm(p => ({ ...p, code: e.target.value.toUpperCase() }))} placeholder="MOB" maxLength={10} dir="ltr" />
            </div>
            <div>
              <label className="block text-sm font-bold text-slate-300 mb-2 ps-2">تصنيف المحفظة *</label>
              <select required className="w-full bg-[#0b0e14] border-none text-white rounded-xl px-4 py-3 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-[#0e7a54]"
          ...
          <div>
            <label className="block text-sm font-bold text-slate-300 mb-2 ps-2">اللون المميز للمحفظة</label>
            <div className="flex gap-3 items-center bg-[#0b0e14] p-2 rounded-xl">
              <input type="color" value={form.color} onChange={e => setForm(p => ({ ...p, color: e.target.value }))}
                className="w-10 h-10 rounded-lg cursor-pointer bg-[#151a23] border border-[#222a36]" />
              <span className="text-[#8b95a7] text-sm font-mono font-bold">{form.color}</span>
            </div>
          </div>
          <div>
            <label className="block text-sm font-bold text-slate-300 mb-2 ps-2">ملاحظات داخلية (اختياري)</label>
            <textarea className="w-full bg-[#0b0e14] border-none text-white rounded-xl px-4 py-3 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-[#0e7a54]" 
          ...
          <div className="flex gap-3 pt-4 border-t border-[#222a36]">
            <button type="button" onClick={() => setOpen(false)} className="flex-1 bg-[#151a23] hover:bg-[#1a212c] text-slate-300 border border-[#222a36] font-bold text-sm px-6 py-3 rounded-xl transition-colors">إلغاء</button>
    ...
    <div className="flex-1 overflow-y-auto px-8 pb-8 space-y-6 bg-[#0b0e14] font-sans text-slate-100" >
      ...
      <div className="bg-[#151a23] rounded-2xl p-6 shadow-sm border border-[#222a36] flex flex-col md:flex-row md:items-center justify-between gap-4 mt-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-white mb-1">المحافظ والمشاريع (Portfolios)</h1>
            <p className="text-[#8b95a7] text-sm">إدارة الشركات والمشاريع المرتبطة بعمليات التحصيل، وتصنيف الديون</p>
          </div>
        </div>
        <div className="flex items-center gap-4 w-full md:w-auto">
          <div className="relative flex-1 md:flex-none">
            <Search className="absolute end-3 top-1/2 -translate-y-1/2 text-[#5f6b7e]" size={18} />
            <input
              type="text"
              className="w-full md:w-64 bg-[#0b0e14] border-none text-white rounded-xl pe-10 ps-4 py-2.5 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-blue-500/20 placeholder:text-[#5f6b7e]"
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-[#151a23] rounded-2xl border border-[#222a36] p-5 shadow-sm hover:shadow-md transition-shadow">
          <div className="text-[#8b95a7] text-xs font-bold mb-2">إجمالي المحافظ</div>
          <div className="flex items-end justify-between">
            <div className="font-bold text-3xl text-white font-mono">{portfolios.length}</div>
            <div className="w-8 h-8 rounded-full bg-[#222a36] flex items-center justify-center text-[#5f6b7e]"><FolderKanban size={16}/></div>
          </div>
        </div>
        <div className="bg-[#151a23] rounded-2xl border border-[#222a36] p-5 shadow-sm hover:shadow-md transition-shadow">
          <div className="text-[#8b95a7] text-xs font-bold mb-2">محافظ نشطة (Active)</div>
          <div className="flex items-end justify-between">
        ...
        {Object.keys(byCategory).slice(0, 2).map((cat, i) => (
          <div key={cat} className="bg-[#151a23] rounded-2xl border border-[#222a36] p-5 shadow-sm hover:shadow-md transition-shadow">
            <div className="text-[#8b95a7] text-xs font-bold mb-2">قطاع {CATEGORY_LABELS[cat as PortfolioCategory]}</div>
            <div className="flex items-end justify-between">
      ...
      {loading ? (
        ...
        <div className="bg-[#151a23] rounded-2xl border border-[#222a36] shadow-sm p-16 text-center">
          ...
          <div className="font-bold text-xl text-white mb-2">لا توجد محافظ مطابقة</div>
          <p className="text-[#8b95a7] text-sm">أضف محفظة جديدة للبدء في ربط الديون والعملاء بها.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filtered.map(portfolio => (
            <div key={portfolio.id} className={`bg-[#151a23] rounded-2xl border shadow-sm p-6 transition-all duration-300 hover:shadow-md flex flex-col ${!portfolio.is_active ? 'opacity-60 bg-[#222a36]/50 border-[#222a36]' : 'border-[#222a36]'}`}>
              ...
              <div className="flex items-start justify-between gap-4 mb-6">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    {portfolio.name_ar && <div className="text-[#5f6b7e] text-xs font-mono" dir="ltr">{portfolio.name}</div>}
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3 mb-6 flex-wrap">
                ...
                <span className="bg-[#0b0e14] text-[#8b95a7] text-xs font-bold px-3 py-1.5 rounded-lg font-mono flex items-center gap-1.5">
                  <Settings size={14} /> {portfolio.source_system === 'manual' ? 'يدوي' : portfolio.source_system}
                </span>
              </div>

              {portfolio.notes && (
                <div className="bg-[#0d1117] border border-[#222a36] rounded-xl p-3 mb-6">
                  <p className="text-[#8b95a7] text-xs font-medium leading-relaxed line-clamp-2">{portfolio.notes}</p>
                </div>
              )}
