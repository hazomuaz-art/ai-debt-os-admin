  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-white/40 text-sm mt-0.5">
            AI System Orchestrator &amp; Auditor â€” {report.generated_at.slice(0, 19).replace('T', ' ')} UTC
          </p>
        </div>
        <div className="flex gap-2">
          <Link href="/api/auditor?safe_fix=true">
            <button className="btn-secondary text-xs px-3 py-2">ðŸ”§ Apply Safe Fixes</button>
          </Link>
          <Link href="/api/orchestrator">
            <button className="btn-secondary text-xs px-3 py-2">ðŸ“Š Run History JSON</button>
          </Link>
          <TriggerBatchButton />
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-6 gap-4">
        ...
        {([
          { label: 'âœ… Working',           value: report.working,           color: '#34d399' },
          { label: 'âš¡ Partial',           value: report.partial,           color: '#fbbf24' },
          { label: 'âŒ Broken',            value: report.broken,            color: '#f87171' },
          { label: 'ðŸ”— Not Connected',     value: report.not_connected,     color: '#818cf8' },
          { label: 'ðŸ”‘ Needs Credentials', value: report.needs_credentials, color: '#a78bfa' },
      ...
      {/* Orchestrator stats */}
      <div className="card p-5">
        <h2 className="section-title mb-4">ðŸ¤– Orchestrator Activity</h2>
              ...
              <tbody>
                {(runs as Array<Record<string, unknown>>).map(run => (
                  <tr key={String(run.id)} className="border-b border-white/[0.03] hover:bg-white/[0.02]">
                    ...
                    <td className="py-2 pr-4 text-white/40">{String(run.mode ?? 'â€”')}</td>
                    <td className="py-2 pr-4" style={{ color: '#818cf8' }}>{run.ai_score != null ? String(run.ai_score) : 'â€”'}</td>
                    ...
                    <td className="py-2 pr-4">
                      <span className={`px-2 py-0.5 rounded-full text-[10px] font-semibold ${
                        ...
                        {run.success ? 'âœ“' : 'âœ—'} {run.success ? 'OK' : String(run.error_message ?? 'Error').slice(0, 25)}
                      </span>
                    </td>
                    <td className="py-2 pr-4 text-white/30 font-mono">{run.duration_ms != null ? `${run.duration_ms}ms` : 'â€”'}</td>
      ...
      {/* Audit sections */}
      <Section title="Pipeline Modules"    items={report.summary.pipeline}     emoji="âš¡" />
      <Section title="Module Connections"  items={report.summary.modules}      emoji="ðŸ”—" />
      <Section title="Integrations"        items={report.summary.integrations}  emoji="ðŸ”Œ" />
      <Section title="Database Tables"     items={report.summary.tables}       emoji="ðŸ—„" />
      <Section title="API Routes"          items={report.summary.api_routes}   emoji="ðŸ›£" />
      <Section title="Dashboard Pages"     items={report.summary.pages}        emoji="ðŸ“„" />

      {/* Safe fixes applied */}
      {report.safe_fixes_applied?.length && (
        <div className="card p-5" style={{ borderColor: 'rgba(16,185,129,0.2)' }}>
          <h2 className="section-title mb-3" style={{ color: '#34d399' }}>âœ… Safe Fixes Applied</h2>
          <ul className="space-y-1">
            {report.safe_fixes_applied.map((fix: string, i: number) => (
              <li key={i} className="text-sm text-white/70 flex items-start gap-2">
                <span className="text-green-400 mt-0.5">âœ“</span>{fix}
              </li>
            ))}
          </ul>
        </div>
      ...
      <div className="text-center pb-4">
        <p className="text-xs text-white/20">
          Auditor reads data only â€” no changes unless you click "Apply Safe Fixes".
          Health score: {report.total_checks} checks Â· {report.working} working Â· {report.broken} broken.
        </p>
      </div>
    </div>
  )
