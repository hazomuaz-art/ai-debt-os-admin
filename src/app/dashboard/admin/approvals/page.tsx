    ...
    <div className="flex-1 overflow-y-auto px-8 pb-8 space-y-6 bg-[#0b0e14] font-sans text-slate-100" >
      ...
      <div className="bg-[#151a23] rounded-2xl p-6 shadow-sm border border-[#222a36] flex items-center justify-between mt-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-white mb-1">الموافقات والتدخل الإداري</h1>
            <p className="text-[#8b95a7] text-sm">مراجعة العمليات الحساسة واعتمادها قبل التنفيذ الآلي</p>
          </div>
        </div>
      </div>
      ...
      {pending.length > 0 && (
        <div className="bg-gradient-to-r from-amber-50 to-orange-50 rounded-2xl p-5 border border-amber-100 flex items-center justify-between shadow-sm animate-in fade-in">
          <div className="flex items-center gap-4">
            <div className="w-10 h-10 bg-[#151a23] rounded-full flex items-center justify-center text-amber-500 shadow-sm">
              <AlertTriangle size={20} />
            </div>
            <div>
              <p className="font-bold text-white">طلبات بانتظار المراجعة ({pending.length})</p>
        ...
        {['all','pending','approved','rejected'].map((s: string) => {
          ...
          return (
            <button key={s} onClick={() => setFilter(s)}
              className={`px-5 py-2 rounded-xl text-sm font-bold transition-all ${isActive ? 'bg-[#0e7a54] text-white shadow-md' : 'text-[#8b95a7] hover:bg-[#1a212c] hover:text-white'}`}>
              {labels[s]}
            </button>
          )
        })}
      ...
      {planModalItem && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4">
          <div className="bg-[#151a23] rounded-3xl shadow-xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-6">
              <h3 className="text-xl font-bold text-white mb-1">تحديد خطة التقسيط</h3>
              <p className="text-sm text-[#8b95a7] mb-6">يرجى إدخال تفاصيل الدفع لاعتمادها وإرسالها للعميل.</p>

              <div className="space-y-5">
                <div>
                  <label className="block text-sm font-bold text-slate-200 mb-1.5">عدد الأقساط</label>
                  <select 
                    value={planData.count} 
                    onChange={e => setPlanData({ ...planData, count: Number(e.target.value) })}
                    className="w-full bg-[#222a36] border border-[#222a36] text-slate-100 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-[#0e7a54]/20"
                  >
                ...
                <div>
                  <label className="block text-sm font-bold text-slate-200 mb-1.5">فترة السداد</label>
                  <select 
                ...
                <div>
                  <label className="block text-sm font-bold text-slate-200 mb-1.5">الدفعة الأولى المطلوبة للبدء</label>
                  <div className="relative">
                    <input 
                      ...
                      className="w-full bg-[#222a36] border border-[#222a36] text-slate-100 rounded-xl px-4 py-3 pe-12 focus:outline-none focus:ring-2 focus:ring-[#0e7a54]/20"
                    />
            ...
            <div className="bg-[#222a36] p-4 border-t border-[#222a36] flex items-center justify-end gap-3">
              <button onClick={() => setPlanModalItem(null)} className="px-5 py-2.5 rounded-xl font-bold text-[#8b95a7] hover:bg-slate-200/50 transition-colors">
                إلغاء
              </button>
              <button 
