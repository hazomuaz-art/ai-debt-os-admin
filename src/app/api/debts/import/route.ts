  ...
  try {
    ...
    for (const cluster of clusters) {
      ...
      clusterRuntimes.push({
        ...
        unresolvedFields: Object.entries(resolutions).filter(([, r]) => r.needsMapping).map(([f]) => f),
        headerFieldMap,
      })
    ...
    const needsMappingReport = needsMappingClusters.map(({ cr, i }) => ({
      ...
      unresolved_fields: cr.unresolvedFields,
      row_numbers: [...rowToCluster.entries()].filter(([, ci]) => ci === i).map(([r]) => r + 2),
    }))
