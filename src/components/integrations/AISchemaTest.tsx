      ...
      {result && (
        <div className="grid lg:grid-cols-3 gap-4">
          <div className="card p-4">
            <div className="text-[#8b95a7] text-xs uppercase tracking-wider">Confidence</div>
            <div className="font-display text-2xl font-bold text-green-400">
              {Math.round((result.confidence ?? 0) * 100)}%
            </div>
            <div className="text-[#5f6b7e] text-xs mt-1">
              Project: {result.detected_project_type ?? 'unknown'}
            </div>
          </div>

          <div className="card p-4 lg:col-span-2">
            <div className="text-[#8b95a7] text-xs uppercase tracking-wider mb-3">Field Mapping</div>
            <div className="space-y-2">
              {Object.entries(result.field_mapping ?? {}).map(([source, target]) => (
                <div key={source} className="flex items-center justify-between gap-3 text-sm border-b border-[#222a36] pb-2">
                  <span className="text-[#8b95a7]">{source}</span>
          ...
          <div className="card p-4 lg:col-span-3">
            <div className="text-[#8b95a7] text-xs uppercase tracking-wider mb-3">Status Mapping</div>
            {Object.keys(result.status_mapping ?? {}).length === 0 ? (
              ...
              <div className="grid md:grid-cols-2 gap-3">
                {Object.entries(result.status_mapping).map(([source, info]) => (
                  <div key={source} className="rounded-xl bg-[#222a36] border border-[#222a36] p-3">
