...
export async function generateOpeningMessage(args: {
  ...
  const fallback = `السلام عليكم${name}، معك خالد من قسم التحصيل بخصوص مديونيتك القائمة. متى تقدر تسددها؟`
...
═══ المهمة ═══
اكتب رسالة افتتاح أولى لبدء محادثة واتساب مع هذا العميل (لم تتواصل معه من قبل):
- ابدأ بتحية مناسبة (السلام عليكم).
- عرّف نفسك باسم خالد من قسم التحصيل بخصوص الجهة الدائنة إن وُجدت.
- اذكر مبلغ المديونية مرة واحدة فقط إن كان متوفراً.
- اسأله مباشرة ومتى يقدر يسدد.
- جملتان كحد أقصى، طبيعية وبشرية، بدون "عزيزي العميل".
