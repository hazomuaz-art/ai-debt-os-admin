...
import { resolvePaymentReference } from '@/lib/ai-collector-agent'
import { createLogger } from '@/lib/logger'

// Strips any emoji the model still slips in despite the prompt instruction
// — owner requirement (2026-07-11): emoji are banned outright in campaign
// messages as an anti-ban precaution, not just discouraged. A prompt-only
...
export async function generateCampaignMessage(args: {
  ...
  try {
    ...
    // Owner requirement (2026-07-11): give the customer their real payment
    // number, not the internal file reference. Reuses the SAME deterministic,
    // safety-critical resolver ai-collector-agent.ts uses for live replies
    // (resolvePaymentReference/resolveMobilyPaymentNumber) — Mobily's correct
    // payment number depends on service status (Inactive→service number,
    // Closed→account number), and giving the wrong one can post a payment to
    // the wrong account. Never left to the model to pick from raw data.
    const portfolioId = (ctx.debt as any)?.portfolio_id ?? null
    let mobilyRow: Record<string, any> | null = null
    if (portfolioId) {
      const { data: portfolioRow } = await svc.from('portfolios').select('metadata').eq('id', portfolioId).maybeSingle()
      const companyKey = (portfolioRow as any)?.metadata?.company_key ?? null
      if (companyKey === 'mobily') {
        const { data: rows } = await svc.from('customer_data_mobily').select('*').eq('customer_id', args.customer_id)
        mobilyRow = (rows ?? [])[0] ?? null
      }
    }
    const paymentNumber = resolvePaymentReference(ctx, mobilyRow)
    ...
    const facts = [
      ...
      (d as any).product_type ? `نوع المنتج: ${(d as any).product_type}` : null,
      (d as any).account_number ? `رقم الحساب: ${(d as any).account_number}` : null,
      paymentNumber ? `رقم السداد الفعلي (استخدم هذا، وليس أي رقم آخر): ${paymentNumber}` : null,
      money((d as any).current_balance) ? `الرصيد المستحق: ${money((d as any).current_balance)}` : null,
      (d as any).due_date ? `تاريخ التعثر: ${(d as any).due_date}${daysOverdue > 0 ? ` (متأخر ${daysOverdue} يوم)` : ''}` : null,
    ...
    const firstMessageRuleAr = isFirstMessage
      ? `🔴 هذه أول رسالة لهذا العميل بخصوص هذا الملف — يجب أن تذكر بوضوح: ${hasName ? 'اسم العميل الحقيقي (خاطبه به في التحية مباشرة، إلزامي وليس اختيارياً)، ' : ''}الجهة الدائنة، نوع المنتج، المبلغ بالضبط، رقم السداد الفعلي (إن وُجد — استخدمه هو تحديداً، لا الرقم المرجعي الداخلي)، ومنذ متى الدين متأخر (تاريخ التعثر). لا تكتفِ بسطر عام؛ العميل يحتاج يعرف تفاصيل المطالبة كاملة ليقدر يتحقق منها ويتصرف. يمكن أن تكون الرسالة 3-4 أسطر إن احتاج الأمر لذكر كل هذا بوضوح، بنفس الأسلوب السعودي الطبيعي.`
    ...
    const firstMessageRuleEn = isFirstMessage
      ? `This is the FIRST message to this customer about this claim — it must clearly state: ${hasName ? "the customer's real name (address them by it directly in the greeting, mandatory, not optional), " : ''}the creditor, the product type, the exact amount, the real payment number (if available — use that specifically, not the internal file reference), and how long it has been overdue (default date). Do not send a vague one-liner; the customer needs the full claim details to verify and act on it. The message can run 3-4 short sentences if needed to cover this clearly, in a natural, professional, human tone.`
...
- 🔴🔴 ممنوع استخدام أي إيموجي إطلاقاً، ولا حتى واحد — لا في التحية ولا في أي مكان من الرسالة.
...
- 🔴🔴 Never use any emoji, not even one — not in the greeting, not anywhere in the message.
...
Return only the message text, no explanation or quotation marks.`
    ...
    const text = stripEmoji((ai.choices[0]?.message?.content ?? '').trim().replace(/^["'«]|["'»]$/g, '').trim())
