...
function looksLikeName(v: string): boolean {
  ...
  // Mostly letters (Arabic or Latin), allow short connectors.
...
function contentScore(field: StandardField, values: string[]): number {
  ...
  switch (field) {
    case 'full_name': {
      ...
      // Names are free text AND almost always unique per row. A column that
      // is name-shaped but highly repetitive is much more likely a
      // status/category field that merely happens to contain words — so we
      // require both signals, which is what generally tells "اسم العميل"
      // apart from "حالة العميل"/"تصنيف العميل" without naming either.
      return shapeRatio * 0.6 + uniq * 0.4
    }
    case 'national_id':
