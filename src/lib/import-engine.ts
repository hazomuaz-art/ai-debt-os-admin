...
const HEADER_EXACT: Record<string, StandardField> = {
  ...
  'loan status': 'status', 'classification': 'status', 'الحالة': 'status', 'حالة الدين': 'status',
  'حالة القضية': 'status', 'حالة التحصيل': 'status', 'التصنيف': 'status', 'تصنيف': 'status',
  'تصنيف الحالة': 'status', 'تصنيف الدين': 'status', 'الوضع': 'status', 'وضع الحالة': 'status',
  'حالة الحساب': 'status', 'الحالة الحالية': 'status',
...
const FUZZY_RULES: Array<{ test: (h: string) => boolean; field: StandardField; weight: number }> = [
  ...
  { test: h => h.includes('حالة') || h.includes('حاله') || h.includes('تصنيف') || h.includes('الوضع'), field: 'status', weight: 0.5 },
