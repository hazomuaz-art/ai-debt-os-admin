...
- تربط الكلام بمصلحة العميل (تجنّب التصعيد القانوني، تفادي زيادة المبلغ، إغلاق الملف وراحة باله) لا بمجرد المطالبة.
...
إن كانت رسالتك الأخيرة سؤالاً وقد أجاب العميل عليه الآن، لا تعد السؤال — انتقل بالمحادثة للأمام.`,
  ...
  // ════════════════════════════════════════════════════════════════════
  {
    ...
    // (K2) Insurance dispute — NEVER resolved automatically. Any objection
    // to the recourse reason / fault percentage, or any sign the customer
    // is contesting the claim with counter-evidence ("حذف مسترد" trigger),
    // is routed to human review unconditionally — regardless of what the
    // model decided. The model NEVER gets to confirm or drop a claim; this
    // overrides its action/message outright, it is not a text suggestion.
    if (playbook.category === 'insurance' && insuranceObjection) {
      if (insuranceObjection.contradictsClaimReason) {
        log.warn('insurance claim contested with counter-evidence — forcing document review, no AI resolution', { portfolio_id: resolvedPortfolioId })
        parsed.action = 'human_review'
        parsed.reason = 'recovered_deduction_review'
        parsed.message = 'تمام، هذا يحتاج مراجعة من قسم المطالبات على المستند اللي ذكرته — راح يتواصلون معك بعد المراجعة. تقدر ترسل المستند الآن لو جاهز عندك؟'
      } else if (insuranceObjection.objectsToRecourseOrFault) {
        log.warn('insurance objection to recourse reason/fault percentage — forcing review, no AI resolution', { portfolio_id: resolvedPortfolioId })
        parsed.action = 'human_review'
        parsed.reason = 'insurance_dispute_review'
        parsed.message = 'تمام، نقطة سبب الرجوع/نسبة الخطأ تحتاج مراجعة من المختصين عندنا — راح أرفع ملاحظتك ويتواصلون معك.'
      }
      parsed.promised_date = null
      parsed.promise_text = null
    }
