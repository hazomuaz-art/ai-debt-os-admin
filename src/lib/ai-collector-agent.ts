...
export async function runCollectorAgent(args: {
  ...
  // ── Intent router ──
  type AgentIntent = 'GREETING' | 'SELF_INTRO' | 'INTRODUCTION' | 'INFO_REQUEST' | 'NEGOTIATION' | 'DISPUTE' | 'WRONG_NUMBER' | 'GENERAL'
...
إن كانت رسالتك الأخيرة سؤالاً وقد أجاب العميل عليه الآن، لا تعد السؤال — انتقل بالمحادثة للأمام.`,
  ...
  if (signals.wrongNumber && parsed.action !== 'record_wrong_number') {
