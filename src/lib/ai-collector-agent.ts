...
function detectSignals(text: string) {
  return {
    ...
    // Customer themselves invokes a lawyer/court — distinct from refusesToPay
    // (which is about refusing to pay at all): this fires the immediate
    // lawyer-persona switch (see bansLegalTone gate below), not just the
    // 3-refusals/48h pattern.
    invokesLegalChallenge: hasAny(text, [
      'محامي', 'محاميي', 'وكيلي القانوني', 'هات محاميكم', 'وين محاميكم',
      'ارفعوها للمحكمة', 'ارفعوها للمحكمه', 'ارفعها للمحكمة', 'ارفعها للمحكمه',
      'خذها للمحكمة', 'خذها المحكمة', 'شوف المحكمة', 'وريك المحكمة',
      'هرفع شكوى', 'برفع شكوى', 'بشتكي عليكم', 'بقاضيكم', 'هقاضيكم',
    ]),
...
export async function runCollectorAgent(args: {
  ...
  if (forcedDebtId) {
    ...
    // Customer invokes a lawyer/court THEMSELVES, right now — switch to the
    // lawyer persona immediately rather than waiting for the
    // 3-refusals/48h pattern (trackRefusalForLegalEscalation below still
    // also runs for the slower pattern). Banned entirely for the same 3
    // portfolios excluded from legal tone everywhere else.
    if (signals.invokesLegalChallenge && !bansLegalTone) {
      const d = ctx.verified_debt_data ?? {}
      const caseSummary = `الجهة: ${d.creditor_name ?? d.portfolio_name ?? 'غير محدد'} | المبلغ المتأخر: ${d.current_balance ?? '—'} ${d.currency ?? 'SAR'} | الرقم المرجعي: ${d.reference_number ?? '—'}`
      log.info('customer invoked legal challenge — switching to lawyer persona immediately', { debt_id: forcedDebtId })
      const lawyerReply = await generateLawyerPersonaReply({
        customerMessage: text, caseSummary, reason: 'العميل طلب التصعيد القانوني أو ذكر محامياً/محكمة بنفسه',
      })
      return {
        shouldReply: true, action: 'human_review',
        reason: 'customer_invoked_legal_challenge', message: lawyerReply,
        resolvedDebtId: forcedDebtId,
      }
    }
...
- **تصغير الخطوة المطلوبة**: عميل متردد أو يرى المبلغ كبيراً؟ لا تطلب الحل الكامل دفعة واحدة — اطلب أصغر التزام ممكن الآن (تاريخ محدد، أو صورة إيصال جزئي) بدل "سدد كامل المبلغ".
- **الربط بمصلحته المباشرة لا بمصلحتك**: لا تقل "أنا محتاج أقفل الملف" — قل "هذا يخلصك من المتابعة المستمرة ويريحك".
- **تغيير زاوية الطرح عند التكرار**: لو رفض أو ماطل على نفس الطلب، لا تُعد صياغته بكلمات مختلفة فقط — غيّر الزاوية كلياً (مرة اسأل عن السبب، مرة اعرض حلاً بديلاً، مرة اطلب التزاماً جزئياً أصغر).
