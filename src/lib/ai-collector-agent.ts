...
export type CollectorDecision = {
  ...
  // Real production incident (customer 057da61b, 2026-07-09): the customer
  // explained "تم التواصل معهم واتضح عدم وجود عقد وتم تقديم طلب عدم ملكية
  // وسيتم التواصل من قبلهم خلال 5 أيام عمل" (contacted the company, no
  // contract exists, filed a non-ownership dispute, THE COMPANY will
  // respond within 5 business days) — describing the CREDITOR'S own
  // response window while actively disputing the debt, not committing to
  // pay anything. The deterministic hasTemporalRef() backstop below (built
  // to catch cases where the model fails to recognize a genuine payment
  // commitment) is a bare lexical scan for ANY time-like phrase in the
  // text — it cannot tell whose timeframe is being described, so it forced
  // action=record_promise and fabricated a payment date + number for a
  // customer who never promised anything. Same root cause as
  // dispute_reason/legal_escalation_trigger above: whether a message
  // contains a genuine personal payment commitment is a judgment only real
  // understanding can make, never a keyword/lexicon match. The model
  // reports its own verdict here EVERY turn (not just when action is
  // already record_promise) — true only when the CUSTOMER THEMSELVES, in
  // this current message, commits to pay by some time; false when any
  // time reference present belongs to someone/something else (the
  // creditor's response window, a third party, an appointment, a general
  // date mention unrelated to payment). The deterministic guards below
  // treat an explicit `false` as decisive — never force or accept a
  // promise over it.
  customer_commits_to_pay?: boolean | null
...
13.3 🔴🔴 حادثة حقيقية: عميل قال "تم التواصل معهم واتضح عدم وجود عقد وتم تقديم طلب عدم ملكية وسيتم التواصل من قبلهم خلال 5 أيام عمل" — يتحدث عن رد **الشركة الدائنة نفسها** على اعتراضه، وليس التزاماً منه بالسداد، لكن النظام (بالخطأ سابقاً) سجّلها "وعد سداد" وأعطاه رقم دفع بسبب وجود "خلال 5 أيام عمل" في كلامه. **أي إشارة زمنية في رسالة العميل ليست تلقائياً وعد سداد** — افهم لمين يعود هذا التوقيت: هل العميل نفسه ملتزم بالسداد خلاله، أم يتحدث عن موعد رد الشركة/جهة أخرى/متابعة إدارية لا علاقة لها بأنه سيدفع؟ عبّئ "customer_commits_to_pay" بصدق حسب هذا الفهم — لا تجعلها true لمجرد وجود تاريخ أو مدة في الرسالة.
...
15. 🔴🔴 حادثة حقيقية: عميل قال "ماعندي رقم لدى موبايلي ولا سبق اخذت منهم" (نفي واضح ومتسق لوجود أي علاقة بالجهة، مطابق تماماً لكلامه من البداية)، فرد الوكيل "رفعت طلب عدم الملكية عندهم فعلياً، وهذا يخالف كلامك الحين إنك ماعندك رقم أصلاً" — متهماً العميل بالتناقض رغم أن كلامه متسق تماماً مع طلب عدم الملكية نفسه (طلب عدم الملكية هو بالضبط الإجراء المناسب لمن يقول "ماعندي رقم"، لا تناقض بينهما أصلاً). هذا اختلاق تناقض غير موجود، أرهق العميل واضطره يكرر نفي نفس الشيء عدة مرات. **ممنوع منعاً باتاً اتهام العميل بمناقضة نفسه أو كتابة أي جملة فيها "هذا يخالف كلامك" أو "يناقض ما قلته"** إلا إذا كان هناك تناقض حرفي واضح بين كلامه الحالي وكلام سابق له هو نفسه بنفس المحادثة (لا بين كلامه وإجراء اتخذته أنت، ولا بين كلامه ورسالة سابقة أرسلتها أنت). قبل كتابة أي اتهام بالتناقض، قارن حرفياً بين الجملتين المعنيتين من كلام العميل نفسه — لو ما لقيت تناقضاً حرفياً واضحاً، احذف الاتهام تماماً ورد على مضمون كلامه مباشرة.

═══════════════ صيغة الإخراج ═══════════════
أعد JSON فقط بهذا الشكل، بدون أي نص خارجه. 🔴 التزم بترتيب الحقول هذا بالضبط — "message" يجي فور بعد "action"، قبل أي حقل آخر: لو انقطع ردك لأي سبب (حد التوكينات مثلاً)، هذا الترتيب يضمن أن رد العميل نفسه وصل كاملاً قبل أي انقطاع، بينما بقية الحقول (تفصيلية/داخلية) هي اللي تُفقد لا رسالة العميل:
{
  "shouldReply": true,
  ...
  "customer_commits_to_pay": "true أو false — املأها في كل رسالة فيها أي إشارة زمنية. true فقط إذا كان العميل نفسه، في رسالته الحالية، يلتزم شخصياً بالسداد خلال وقت معيّن. false إذا كان أي توقيت مذكور يخص طرف آخر أو موضوعاً آخر (رد الشركة الدائنة نفسها، جهة تحقيق، موعد إداري، تاريخ عام لا علاقة له بالتزام سداد منه) — راجع القاعدة 13.3."
}
...
🔴 تذكير أخير لا تنساه: لا تخترع بيانات، لا تكرر سؤالاً مُجاباً، التزم بما اتُّفق عليه، وردك قصير وبشري.`
  ...
  try {
    ai = await client.chat.completions.create({
      model: modelId,
      temperature: 0.6,
      // Was 400, then 600 — still not enough headroom: a real production
      // completion (customer 4a47f571, 2026-07-09) got cut off mid-JSON
      // after only ~6 of the schema's 10 fields. Root cause is Arabic
      // token inefficiency, not verbosity — this schema always emits Arabic
      // text in "reason" (never optional) plus up to 4 more optional string
      // fields (dispute_reason, promise_text, etc.), and BPE tokenizers
      // split Arabic into roughly 2-3x more tokens per character than
      // English/Latin script, so even a "short" Arabic sentence can burn
      // through a budget sized for English word counts. Raised for real
      // headroom instead of chasing the symptom. Independently, the schema
      // field order below now puts "message" (the only field that reaches
      // the customer) right after "action" — so even if a future model/
      // prompt combination still runs out of budget, the customer-facing
      // text is captured before the cutoff, and only internal bookkeeping
      // fields are lost.
      max_tokens: 900,
      response_format: { type: 'json_object' },
      messages: [
...
إن كانت رسالتك الأخيرة سؤالاً وقد أجاب العميل عليه الآن، لا تعد السؤال — انتقل بالمحادثة للأمام.`,
  ...
  const finishReason = (ai.choices[0] as any)?.finish_reason
  const obj = extractJson(raw)
  let parsed: CollectorDecision

  // Diagnostic for the next occurrence — root-caused this once from a single
  // stored production message with no visibility into WHY the budget ran
  // out (finish_reason/token usage aren't persisted anywhere). Logging them
  // whenever a completion hit the length cap turns any recurrence into an
  // immediate, provable answer instead of another guessing exercise.
  if (finishReason === 'length') {
    log.warn('completion hit max_tokens (finish_reason=length) — response may be truncated', {
      intent, max_tokens: 900, completion_tokens: (ai as any).usage?.completion_tokens, raw_length: raw.length,
    })
  }
  ...
  //
  // 🔴 Real production incident (customer 057da61b, 2026-07-09): a bare
  // lexical time-reference in the text ("خلال 5 أيام عمل") describing the
  // CREDITOR'S OWN response window to a dispute the customer had just filed
  // ("اتضح عدم وجود عقد وتم تقديم طلب عدم ملكية") got forced into
  // record_promise anyway, fabricating a payment date + number for a
  // customer who never committed to pay anything. `customer_commits_to_pay`
  // (§13.3) is the model's own semantic verdict on WHOSE timeframe this is
  // — an explicit `false` is decisive and blocks the force, exactly like
  // the existing deniesDebt/deniesPromise/notAGenuineCommitment checks.
  // Undefined (model didn't return the field) preserves prior behavior so
  // the original real-promise-detection cases this guard was built for
  // still work.
  let promiseForcedFromTemporalRef = false
  if ((hasTemporalRef(text) || !!engineResolution?.resolved) && !negatesImmediateTiming && parsed.action !== 'record_promise' && !signals.deniesDebt && !signals.deniesPromise && !notAGenuineCommitment && parsed.customer_commits_to_pay !== false) {
  ...
  // ════════════════════════════════════════════════════════════════════
  {
    ...
    // due-date reminder, or "وعدت/المهم تسدد"
    // push tacked onto an otherwise correct answer, unless the customer's
    // OWN current message also asked about payment/timing. This is exactly
    // what slipped through before: the model answered "هل هذي كل
    // التفاصيل؟" correctly then appended "والمهم موعدك بكرة 25/6 للسداد" on
    // its own initiative.
    //
    // 🔴 Real production incident (customer 64eb6162, 2026-07-08): asked
    // "ايش مين انت" (a plain INFO_REQUEST identity question) and got "...
    // المبلغ 276.24 ريال، وش رايك نحدد له تاريخ سداد؟" — a payment-date ask
    // tacked onto the identity answer, in direct violation of the
    // INFO_REQUEST rule ("ممنوع منعاً باتاً إضافة أي ضغط سداد..."). The old
    // PRESSURE_PATTERN never matched this exact phrasing (no fixed phrase
    // list can enumerate every way to ask "شو رأيك نحدد تاريخ؟"), and
    // separately had a latent bug: reusing the same /g-flagged RegExp
    // object across multiple .test() calls carries lastIndex state between
    // calls, silently skipping matches. Fixed here two ways: (1) the regex
    // is now stateless (no /g flag — .test()/.exec() on a single string
    // each time, always from index 0); (2) added a general SHAPE check
    // alongside the fixed phrases — any sentence that both ends in "؟" and
    // mentions payment/date vocabulary is a payment question, regardless of
    // its exact wording, which generalizes far past any specific phrase list.
    if (intent === 'INFO_REQUEST' && !signals.promise && !hasTemporalRef(text) && parsed.action !== 'record_promise') {
      const PRESSURE_PATTERN = /(المهم[^.؟!]*?(موعد|تسدد|سداد)|موعدك[^.؟!]*|متى بتسدد[^.؟!]*\??|تقدر تسدد[^.؟!]*\??|جهزت المبلغ[^.؟!]*\??|بانتظار سدادك[^.؟!]*|وعدت[^.؟!]*)/
      const isPaymentQuestionShape = (sentence: string) =>
        /[؟?]\s*$/.test(sentence.trim()) && /(تاريخ|موعد|سداد|تسدد|يسدد|دفع|تحويل)/.test(sentence)
      const hasPressure = PRESSURE_PATTERN.test(parsed.message)
        || parsed.message.split(/(?<=[.؟!])\s+/).some(isPaymentQuestionShape)
      if (hasPressure) {
        const cleaned = parsed.message
          .split(/(?<=[.؟!])\s+/)
          .filter(sentence => !PRESSURE_PATTERN.test(sentence) && !isPaymentQuestionShape(sentence))
          .join(' ')
          .trim()
        if (cleaned) {
