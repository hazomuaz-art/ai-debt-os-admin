...
export type EscalationRule = { keywords: string[]; reason: string }
...
export type Playbook = {
  ...
  notes: string | null
  // Real free-text/structured policy — read by the agent before every
  // reply (renderPlaybookForPrompt) and enforced where possible by code
  // (forbidden_phrases is a deterministic post-reply guard, not just a
  // prompt suggestion).
...
export async function getPlaybookForPortfolio(params: {
  ...
  return {
    ...
    company_policy: data.company_policy ?? null,
    ai_instructions: data.ai_instructions ?? null,
    forbidden_phrases: Array.isArray(data.forbidden_phrases) ? data.forbidden_phrases : [],
    escalation_rules: Array.isArray(data.escalation_rules) ? data.escalation_rules : [],
    portfolio_specific_rules: data.portfolio_specific_rules ?? null,
    is_default: false,
  }
}
...
export function renderPlaybookForPrompt(pb: Playbook): string {
  ...
  if (pb.company_policy) {
    lines.push('')
    lines.push('【 سياسة الشركة 】')
    lines.push(pb.company_policy)
  }
  if (pb.ai_instructions) {
    lines.push('')
    lines.push('【 تعليمات خاصة بك كوكيل لهذي المحفظة 】')
    lines.push(pb.ai_instructions)
  }
  if (pb.portfolio_specific_rules) {
    lines.push('')
    lines.push('【 قواعد خاصة بهذي المحفظة 】')
    lines.push(pb.portfolio_specific_rules)
  }
  if (pb.forbidden_phrases?.length) {
    lines.push('')
    lines.push('【 🔴🔴 ممنوع منعاً باتاً قول أي من هذي العبارات أو ما يعادلها بالمعنى 】')
    pb.forbidden_phrases.forEach(p => lines.push(`- "${p}"`))
  }
  return lines.join('\n')
}
