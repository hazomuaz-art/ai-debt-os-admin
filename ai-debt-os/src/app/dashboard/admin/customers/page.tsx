    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-white/40 text-sm">{count ?? 0} total customers</p>
        </div>
        <CreateCustomerModal />
      </div>
      ...
      <div className="card overflow-hidden">
        <table className="w-full">
          ...
          <tbody className="divide-y divide-white/5">
            {(customers ?? []).length === 0 ? (
              <tr>
                <td colSpan={6} className="px-4 py-12 text-center text-white/30">No customers yet.</td>
              </tr>
            ) : (customers ?? []).map(c => (
              <tr key={c.id} className="hover:bg-white/2 transition-colors">
                ...
                <td className="px-4 py-3">
                  <div className="text-sm text-white/60">{c.phone ?? c.email ?? '—'}</div>
                  {c.whatsapp && <div className="text-xs text-green-400/60">WA: {c.whatsapp}</div>}
                </td>
                <td className="px-4 py-3">
                  <span className="font-mono text-xs text-white/40">{c.national_id ?? '—'}</span>
                </td>
                ...
                <td className="px-4 py-3">
                  <span className="text-sm text-white/60">{c.city ?? '—'}</span>
                </td>
                <td className="px-4 py-3">
                  <span className="text-sm text-white/40">{formatDate(c.created_at)}</span>
                </td>
              </tr>
            ))}
          </tbody>
