...
const ACTION_COLORS: Record<string, string> = {
  skip_ai:          'bg-white/5 text-white/50',
  ...
  do_nothing:       'bg-white/5 text-white/30',
}
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-white/40 text-sm mt-0.5">قواعد تحدد متى يُستخدم AI ومتى يُستخدم رد محفوظ</p>
        </div>
      ...
      {loading ? <div className="text-center text-white/40 py-12">جارٍ التحميل…</div> : (
        <div className="space-y-2">
          {rules.length === 0 && <div className="card p-10 text-center text-white/40">لا توجد قواعد حتى الآن</div>}
          {rules.map(rule => (
            <div key={rule.id} className={`card p-4 flex items-start justify-between gap-4 ${!rule.is_active ? 'opacity-50' : ''}`}>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 flex-wrap mb-1">
                  ...
                  <span className="text-white/30 text-xs">أولوية: {rule.priority}</span>
                </div>
                {rule.description && <p className="text-white/40 text-xs">{rule.description}</p>}
