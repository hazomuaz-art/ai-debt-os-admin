...
const apiCallOldRegex = /const ai = await client\.chat\.completions\.create\(\{\n\s*model: process\.env\.OPENROUTER_API_KEY \? 'google\/gemini-3\.1-pro-preview' : 'gpt-4o',\n\s*temperature: 0\.45,\n\s*max_tokens: 420,\n\s*response_format: \{ type: 'json_object' \},\n\s*messages: \[\s*\{\s*role: 'system',\s*content: `[\s\S]*?`\.trim\(\),\s*\},/m

const apiCallNew = `const ai = await client.chat.completions.create({
          ...
          parameters: {
            ...
            properties: {
              action_type: {
                ...
                description: "نوع الإجراء المطلوب تسجيله في النظام. استخدم silent إذا انتهت المحادثة ولم يعد هناك داع للرد."
              },
              note: {
                type: "string",
                description: "ملاحظة مختصرة توضح تفاصيل طلب العميل إن وجدت."
              }
            },
...
if (apiCallOldRegex.test(content)) {
  content = content.replace(apiCallOldRegex, apiCallNew)
} else {
  console.log("Could not match the AI call regex.")
}


const parseOldRegex = /let decision: Decision[\s\S]*?if \(!decision\.shouldReply\) return \{ reply: '', nextAction: 'silent' \}/m

const parseNew = `let decision: Decision = { shouldReply: true, reply: '', nextAction: 'reply', confidence: 0.9 }
...
if (parseOldRegex.test(content)) {
  content = content.replace(parseOldRegex, parseNew)
} else {
  console.log("Could not match the parsing logic regex.")
}

fs.writeFileSync(filePath, content, 'utf8')
console.log('Successfully updated to use Tools API and the new Khalid prompt.')
