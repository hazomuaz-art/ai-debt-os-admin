...
vi.mock('@/lib/supabase/server', () => ({
  createServiceClient: vi.fn().mockImplementation(() => ({
    from: vi.fn().mockImplementation(() => ({ insert: mockAlertInsert })),
  })),
}))
...
describe('Insurance Engine (Phase 3)', () => {
  ...
  it('(DD) objection to fault percentage/recourse reason is ALWAYS forced to human_review, even if the model tries to resolve it itself', async () => {
    mockPlaybook = insurancePlaybook()
    mock360 = insuranceGroup({ recourse_reason: 'مخالفة شرط', fault_percentage: 70, accident_date: '2026-01-01' })
    mockContext.recent_promises = []
    mockModelContent = JSON.stringify({ shouldReply: true, action: 'reply', reason: 'model_resolved_it', message: 'لا تقلق، نسبة الخطأ صحيحة ومؤكدة 100%، المطالبة سارية بدون أي مراجعة.', promised_date: null })
    const d = await runCollectorAgent({ company_id: 'c', customer_id: 'u', message: 'ليش نسبة الخطأ علي 70%؟ ما وافقت على هذا' })

    expect(d.action).toBe('human_review')
    expect(d.reason).toBe('insurance_dispute_review')
    expect(d.message).not.toMatch(/مؤكدة|بدون أي مراجعة/)
  })

  it('(EE) customer claims counter-evidence ("حذف مسترد" trigger) -> document review, AI never confirms the claim falls', async () => {
    mockPlaybook = insurancePlaybook()
    mock360 = insuranceGroup({ recourse_reason: 'لا يملك رخصة قيادة سارية', fault_percentage: 100, accident_date: '2026-01-01' })
    mockContext.recent_promises = []
    mockModelContent = JSON.stringify({ shouldReply: true, action: 'reply', reason: 'model_decided', message: 'تمام، بناءً على كلامك المطالبة سقطت ونحذفها من النظام.', promised_date: null })
    const d = await runCollectorAgent({ company_id: 'c', customer_id: 'u', message: 'هذا غير صحيح، رخصتي سارية وبترسل لك صورتها' })

    expect(d.action).toBe('human_review')
    expect(d.reason).toBe('recovered_deduction_review')
